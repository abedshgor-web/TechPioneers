...
router.patch('/:id/status', requireAuth, (req: AuthedRequest, res) => {
  ...
  const campaign = db
    ...
    | { id: string; budget_total: number; bid_amount: number }
    | undefined;
  if (!campaign) {
    return res.status(404).json({ error: 'الحملة غير موجودة' });
  }

  // تسهيل: الإطلاق يتطلب رصيداً يكفي البدء فقط (نقرة واحدة)، لا الميزانية الكاملة
  // مقدماً — فالنظام يوقف الحملة تلقائياً عند نفاد الرصيد/بلوغ الميزانية
  // (حواجز الأمان في serve.ts — راجع docs/ads-platform/03).
  if (status === 'active') {
    const row = db
      .prepare('SELECT COALESCE(SUM(amount), 0) AS bal FROM wallet_ledger WHERE user_id = ?')
      .get(req.user!.sub) as { bal: number };
    const required = Math.max(campaign.bid_amount, 1);
    if (row.bal < required) {
      return res.status(402).json({ error: 'الرصيد غير كافٍ لبدء الحملة — يرجى شحن المحفظة' });
    }
  }
